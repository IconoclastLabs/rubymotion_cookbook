class RootController < UIViewController
  def viewDidLoad
    ...
    if photo_library_available?
      @label.text = "Library is good to go"
      p @label.text
      ...
      media_types = []
      media_types += [KUTTypeImage] if pick_photos_from_library?
      media_types += [KUTTypeMovie] if pick_videos_from_library?
      controller.mediaTypes = media_types
      controller.delegate = self

      self.navigationController.presentModalViewController(controller, animated:true)
    else
      @label.text = "Library not Available"
      p @label.text
    end
  end
